...
public class WaterController : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        probes = GameObject.FindGameObjectsWithTag("probe").ToList();
    }
    ...
    void Update()
    {
        ...
        {   if(waveSources[0].activeSelf==true)
                waveSources[0].SetActive(false);
            else
                waveSources[0].SetActive(true);
        }

        if (Input.GetKeyDown("p"))
        {
            if (waveSources[1].activeSelf == true)
                waveSources[1].SetActive(false);
            else
                waveSources[1].SetActive(true);
        }
    }
    ...
    private void UpdateWaterMesh()
    {
        ...
        {
            float x, y, z;
            x = probes[i].transform.position.x;
            z = probes[i].transform.position.z;
            y = RecalculatePointY(probes[i].transform.position);
            probes[i].transform.position = new Vector3(x, y, z);
        }
    }
    ...
    private float RecalculatePointY(Vector3 point)
    {
        ...
        {   //cycle through all of the wave sources
            if (waveSources[i].activeSelf == true)
            {
                ...
                y += 0.1f * Mathf.Sin(dist * 12.0f - localTime);  // apply the first wave}
            }
            else
            {
            }
